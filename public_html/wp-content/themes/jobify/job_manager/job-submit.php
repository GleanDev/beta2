...
<script>
	jQuery(document).ready(function($){
		var gal =0;
		$('.add_gal .add_button').click(function(){
			gal++;
			$('.fieldset-gallery_image' + gal).show();
		});
		
		$('.job-manager-uploaded-files').closest('.gal_element').show();
	});
</script>
	...
	<?php if ( job_manager_user_can_post_job() ) : ?>
		...
		<?php foreach ( $job_fields as $key => $field ) : ?>
			<fieldset class="fieldset-<?php esc_attr_e( $key ); ?>">
				<?php if($field['label'] == 'Description'): $field['label'] = 'JOB DESCRIPTION'; endif; ?>
		...
		<?php if ( $company_fields ) : ?>
			...
			<?php foreach ( $company_fields as $key => $field ) : ?>
				<?php if($key == 'gallery_image'): ?>
				<div class='add_gal'>
					<div class='add_button'>Add another image</div>
				</div>
				<?php $gal = true; ?>
				<?php endif; ?>
				<fieldset class="fieldset-<?php esc_attr_e( $key ); if($gal === 1): echo ' gal_element'; endif;?>">
					<label for="<?php esc_attr_e( $key ); ?>"><?php echo $field['label'] . apply_filters( 'submit_job_form_required_label', $field['required'] ? '' : ' <small>' . __( '(optional)', 'wp-job-manager' ) . '</small>', $field ); ?></label>
					<div class="field <?php echo $field['required'] ? 'required-field' : ''; ?>">
						<?php get_job_manager_template( 'form-fields/' . $field['type'] . '-field.php', array( 'key' => $key, 'field' => $field ) ); ?>
					</div>
				</fieldset>
				<?php if($gal): $gal=1; endif; ?>
			<?php endforeach; ?>
