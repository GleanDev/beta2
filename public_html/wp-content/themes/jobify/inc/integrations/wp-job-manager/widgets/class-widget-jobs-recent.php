	...
	function widget( $args, $instance ) {
		...
		if ( $has_spotlight ) {
			$spotlight2 = new WP_Query( array(
				...
				'search_location' => 'adhahsdahds',
			) );
			$search_location = '1 Utama Shopping Centre Lebuh Bandar Utama Bandar Utama Petaling Jaya Selangor Malaysia';
			$args = array(
				'post_type' => 'job_listing',
				'post_status' => 'publish',
				'featured' => true,
			...
			$spotlight = get_job_listings( apply_filters( 'job_manager_get_listings_args', $args ) );
			$result['html']    = ob_get_clean();
			$result['showing'] = array();
			$result['showing'][] = sprintf( __( 'located in &ldquo;%s&rdquo;', 'wp-job-manager' ), $search_location );
			apply_filters( 'job_manager_get_listings_result', $result, $spotlight );
				...
				<div class="row">
					...
					<div class="job-spotlight col-lg-4 col-md-6 col-sm-12">
						...
						<ul class="bxslider">
							...
							?>
								<li><?php get_template_part( 'content', 'single-job-featured' ); ?></li>
							<?php
