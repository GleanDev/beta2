...
class WP_Resume_Manager_Shortcodes {
			...
			try {
				...
				switch ( $action ) {
					...
					case 'relist' :
						...
						wp_redirect( add_query_arg( array( 'step' => 'preview', 'resume_id' => absint( $resume_id ) ), get_permalink( get_option( 'resume_manager_submit_page_id' ) ) ) );
	...
	public function candidate_dashboard( $atts ) {
		...
		get_job_manager_template( 'candidate-dashboard.php', array( 'resumes' => $resumes->query( $args ), 'max_num_pages' => $resumes->max_num_pages ), 'wp-job-manager-resumes', RESUME_MANAGER_PLUGIN_DIR . '/templates/' );
