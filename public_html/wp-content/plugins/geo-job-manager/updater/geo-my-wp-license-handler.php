...
class GMW_License {
	...
	public function addon_action_links( $links ) {
		...
		$links['deactivate'] = '<em style="color:red">'.__( 'Please deactivate the license key before deactivating the plugin', 'GJM' ).'</em>';
					...
					<form method="post" action="">
						...
						<?php if ( gmw_is_license_valid( $this->license_name ) ) { ?>	
							...
							<div class="gmw-license-wrapper gmw-license-valid-wrapper">
								...
								<span style="font-size: 14px;"><?php _e( 'License Key:', 'GJM' ); ?></span>
								<input 
								...
								<input 
									...
									value="<?php _e( 'Deactivate License Key', 'GJM' ); ?>" />
						...
						<?php } else { ?>
							...
							<div class="gmw-license-wrapper gmw-license-invalid-wrapper <?php echo $class; ?>">
								...
								<input 
									...
									class="regular-text"
									size="30"
									placeholder="<?php _e( 'Enter license key', 'GJM' ); ?>"
									value="<?php if ( !empty($this->licenses[$this->license_name] ) ) echo $this->licenses[$this->license_name]; ?>" />
								...
								<input 
									...
									value="<?php _e( 'Activate License', 'GJM' ); ?>" />
...
function gmw_update_key_api_notices() {
	...
	return $messages = apply_filters( 'gmw_update_key_api_notices', array(
			'activate'				=> __( 'Please enter your license key and click "Activate License". The license key is required in order to receive automatic updated for the plugin.', 'GJM' ),
			...
			'no_key_entered'		=> __( 'No license key entered. Please enter the license key and try again.', 'GJM' ),
			'expired' 				=> __( 'Your license has expired. Please renew your license in order to keep getting its updates and support.', 'GJM' ),
			'no_activations_left' 	=> sprintf( __( 'Your license has no activations left. Click <a %s>here</a> to manager your license activations.', 'GJM' ), 'href="http://geomywp.com/purchase-history/" target="_blank"' ),
			'missing'				=> __( 'Something is wrong with the key you entered. Please verify the key and try again.', 'GJM' ),
			'retrieve_key'			=> sprintf( __( 'Lost or forgot your license key? <a %s >Retrieve it here.</a>', 'GJM' ), 'href="http://geomywp.com/purchase-history/" target="_blank"' ),
			'activation_error'		=> __( 'Your license for %s plugin could not be activated. See error message below!', 'GJM' ),
	) );
...
function gmw_is_license_valid( $addon ) {
	...
	if ( isset( $statuses[$addon] )
			&& $statuses[$addon] !== false
			&& $statuses[$addon] == 'valid'
			&& isset( $licenses[$addon] )
			&& !empty($licenses[$addon] ) )
	{
		return true;
	} else {
		return false;
...
function gmw_update_key_api() {
	...
	if ( !isset( $_POST['gmw_license_key'] ) || empty( $_POST['gmw_license_key'] ) )
		return;
...
function gmw_update_api_notices() {
	...
	if ( !isset( $_GET['gmw_license_status_notice'] ) || empty( $_GET['gmw_license_status_notice'] ) )
		return;	
