...
class GJM_Query_class {
	...
	public function info_window( $type, $post ) {
		...
		return apply_filters( 'gjm_'.$type.'_info_window_content', implode( '', $output ), $output, $post );
	}
	...
	public function global_map( $args ) {
		...
		foreach ( $results->posts as $key => $value ) {
			$results->posts[$key]->url 			= get_permalink( $value->ID );
			$results->posts[$key]->info_window  = $this->info_window( 'gmap', $value );
		}
